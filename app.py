...
def load_user(user_id):
    ...
    with get_db() as conn:
        with conn.cursor(cursor_factory=DictCursor) as cur:
            ...
            if user_row:
                return User(user_id=user_row['id'], username=user_row['username'], password_hash=user_row['password_hash'])
    return None
...
def generate_short_url():
    """Generates a random 6-character string for a short URL."""
        ...
        with get_db() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                if user_row and check_password_hash(user_row['password_hash'], password):
                    user_obj = User(user_id=user_row['id'], username=user_row['username'], password_hash=user_row['password_hash'])
...
def analytics():
    with get_db() as conn:
        with conn.cursor(cursor_factory=DictCursor) as cur:
            cur.execute(
                "SELECT original_url, short_url, clicks FROM urls WHERE user_id = %s ORDER BY id DESC;",
                (current_user.id,)
            )
